...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <?php
     $student =  (isset($_POST['example1_length'])? $_POST['example1_length']: 0);
     $teacher =  (isset($_POST['example1_teacher'])? $_POST['example1_teacher']: 0);
    ?>
    <!-- Main content -->
    <section class="content">
      <!-- ส่วนของเนื้อหาเเต่ละหน้า -->
        <div class="row">
            ...
            <?php if($teacher!=0 && $student!=0){
              
              ?>
        <div class="col-xs-12">
        <a href="../mPDF/report_admin.php?student=<?php echo $student?>&teacher=<?php echo $teacher?>" onclick="basicPopup(this.href);return false" class="btn btn-danger pull-right"><i class="fa  fa-file-pdf-o"></i>&nbsp;PDF</a>
          <a href="../mPDF/excel_admin.php?student=<?php echo $student?>&teacher=<?php echo $teacher?>" target="_blank" class="btn btn-success pull-right" style="margin-right: 5px;">
            ...
            <div class="col-md-12">
            <form action="admin_report.php" method="post" name="form0" >
            <div class="col-md-5">
            <select name="example1_teacher" class="form-control input-sm" aria-controls="example1" onchange="Javascript : submit();">
            <option value="0">---กรุณาเลือกชื่ออาจารย์---</option>
            <?php
              $sql_teacher = "SELECT * FROM  member WHERE ID_type = '1'";
              $result_teacher     = $conn->query( $sql_teacher );
              $i=0;
              
                 while( $row_teacher = $result_teacher->fetch_array(MYSQLI_BOTH) ){
                   $selected_t =  ($_POST['example1_teacher']==$row_teacher['ID_member'])?'selected' : '';
            ?>
                <option value="<?php echo $row_teacher['ID_member']; ?>" <?php echo $selected_t ; ?> ><?php echo $row_teacher['Firstname']." ".$row_teacher['Lastname']; ?></option>
                 <?php } ?>
                 </select>
                 </div>
                 <div class="col-md-5">
            <select name="example1_length" class="form-control input-sm" aria-controls="example1" onchange="Javascript : submit();">
            <option value="0">---กรุณาเลือกชื่อนักศึกษา---</option>
            <?php
            ...
            ?>
              ...
              <table class="table table-bordered">
                ...
                <?php
                if($teacher!=''){
                   $where_type = "WHERE ID_teacher = ".$teacher;
                }else{
                  $where_type = "WHERE ID_teacher = 0";
                }
